...
public class SocketServer {
	...
	String bindHost; // null => loopback (legacy behavior)
	...
	public SocketServer(int portServerListening) {
		this(portServerListening, 20, null);
	}

	public SocketServer(int portServerListening, int threadPoolSize) {
		this(portServerListening, threadPoolSize, null);
	}

	public SocketServer(int portServerListening, int threadPoolSize, String bindHost) {
	...
	public void startServer() {
		...
		try {
			InetAddress addr;
			if (bindHost == null || bindHost.isEmpty() || "loopback".equalsIgnoreCase(bindHost)) {
				addr = Inet4Address.getLoopbackAddress();
			} else if ("0.0.0.0".equals(bindHost) || "*".equals(bindHost)) {
				addr = null; // wildcard
			} else {
				addr = InetAddress.getByName(bindHost);
			}
			serverSocket = new ServerSocket(portServerListening, 30, addr);
			System.out.println("SocketServer: bind " + (addr == null ? "0.0.0.0" : addr.getHostAddress()) + ":" + portServerListening);
