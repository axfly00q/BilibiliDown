...
//	public static ExecutorService ccThreadPool = Executors.newFixedThreadPool(1);// 用于字幕下载
	...
	@Config(key = "bilibili.server.bindHost", note = "http server 监听地址：127.0.0.1 仅本机, 0.0.0.0 局域网可访问", defaultValue = "0.0.0.0")
	public static String serverBindHost = "0.0.0.0";
	@Config(key = "bilibili.web.auth.enable", note = "预留开关：是否启用控制台独立账号密码（已废弃，请使用 B 站账号登录）", defaultValue = "false", valids = { "true", "false" })
	public static boolean webAuthEnable = false;
	@Config(key = "bilibili.web.auth.username", note = "Web 控制台登录用户名", defaultValue = "admin", warning = false)
	public static String webAuthUsername = "admin";
	@Config(key = "bilibili.web.auth.password", note = "Web 控制台登录密码（首次启动若为空则自动生成并打印到日志）", defaultValue = "", warning = false)
	public static String webAuthPassword = "";

	/**
	 * 无头模式：不显示桌面 Swing 主窗口与过渡动画，仅运行 Web 控制台。
	 * 可通过 app.config 或 系统属性 -Dbilibili.headless=true 控制。
	 * 注意：当前 Java 进程仍依赖 AWT 类（用于二维码图像生成等），不能加 -Djava.awt.headless=true。
	 */
	@Config(key = "bilibili.headless", note = "仅运行 Web 控制台、不弹出桌面主窗口", defaultValue = "false", valids = { "true", "false" })
	public static boolean headless = false;
	/**
	 * headless 模式下，Web 服务器就绪后是否自动在默认浏览器中打开控制台页面。
	 */
	@Config(key = "bilibili.headless.autoOpenBrowser", note = "headless 模式下启动后自动在浏览器打开 Web 控制台", defaultValue = "true", valids = { "true", "false" })
	public static boolean autoOpenBrowser = true;

	/**
	 * 持久化运行的本地 HTTP 服务器（Web 控制台用）。
	 * 普通登录/Cookie 刷新如果发现该实例已存在，则直接复用，避免端口冲突。
	 */
	public static volatile SocketServer persistentSocketServer;

	/**
	 * 确保本地 HTTP 服务器已启动（幂等）。供 Web 控制台菜单使用。
	 */
	public static synchronized SocketServer ensureWebServerStarted() {
		if (persistentSocketServer != null) return persistentSocketServer;
		final SocketServer ss = new SocketServer(serverPort, 20, serverBindHost);
		persistentSocketServer = ss;
		new Thread(new Runnable() {
			@Override public void run() { ss.startServer(); }
		}, "Web控制台-server").start();
		// 等待 ServerSocket 绑定就绪
		try { Thread.sleep(250); } catch (InterruptedException ignored) {}
		// 控制台账号密码鉴权已废弃：默认 webAuthEnable=false，如需启用可手工设置并配合 webAuthPassword 使用。
		if (webAuthEnable && (webAuthPassword == null || webAuthPassword.isEmpty())) {
			webAuthPassword = java.util.UUID.randomUUID().toString().replace("-", "").substring(0, 12);
			System.out.println("==============================================");
			System.out.println("Web 控制台已启动: http://" + ("0.0.0.0".equals(serverBindHost) ? "127.0.0.1" : serverBindHost) + ":" + serverPort + "/console/account.html");
			System.out.println("用户名: " + webAuthUsername);
			System.out.println("初始密码: " + webAuthPassword + "  (仅本次会话有效，可在 app.config 设置 bilibili.web.auth.password 持久化)");
			System.out.println("==============================================");
		} else {
			System.out.println("Web 控制台已启动: http://" + ("0.0.0.0".equals(serverBindHost) ? "127.0.0.1" : serverBindHost) + ":" + serverPort + "/console/index.html");
		}
		return ss;
	}
	@Config(key = "bilibili.frontend.lang", note = "前端页面(极验/CookieRefresh)语言, auto/zh-CN/en-US/ja-JP", defaultValue = "auto", valids = { "auto", "zh-CN", "en-US", "ja-JP" })
	public static String frontendLang = "auto";

	/**
	 * 计算传给前端页面的语言代码（auto 时根据 JVM 默认 Locale 推断）。
	 * @return zh-CN / en-US / ja-JP 之一
	 */
	public static String frontendLangCode() {
		String lang = frontendLang;
		if (lang == null || lang.isEmpty() || "auto".equalsIgnoreCase(lang)) {
			String jvm = java.util.Locale.getDefault().getLanguage();
			if (jvm == null) return "zh-CN";
			jvm = jvm.toLowerCase();
			if (jvm.startsWith("zh")) return "zh-CN";
			if (jvm.startsWith("ja")) return "ja-JP";
			if (jvm.startsWith("en")) return "en-US";
			return "zh-CN";
		}
		return lang;
	}
...
//	private static int httpsProxyPort;
	...
	// https://github.com/axfly00q/BilibiliDown/issues/77
