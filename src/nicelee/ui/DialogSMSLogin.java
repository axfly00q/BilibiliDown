	...
	private void queryCaptcha() {
		...
		if (Global.userName == null || Global.userName.isEmpty()) {
			...
			try {
				...
				String url = String.format("http://localhost:%d/static/index.html?token=%s&gt=%s&challenge=%s&type=sms&lang=%s",
						Global.serverPort, token, gt, challenge, Global.frontendLangCode());
