...
public class CookieRefreshThread extends Thread {
	...
	public void run() {
		...
		if (Global.runWASMinBrowser) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					if (Global.persistentSocketServer != null) {
						socketServer = Global.persistentSocketServer;
						return;
					}
			...
			String browseUrl = String.format("http://localhost:%d/cookieRefresh/index.html?timestamp=%d&lang=%s",
					Global.serverPort, csrfInfoObj.optLong("timestamp"), Global.frontendLangCode());
	...
	private void shutDownServerAsyncDelay() {
		if (socketServer == Global.persistentSocketServer) return; // 不关闭全局服务器
		new Thread(new Runnable() {
			@Override
			public void run() {
				try {
