...
//		this.setPreferredSize(dMenuBar);
		 ...
		 * 创建二级 操作 子菜单
		 */
		JMenuItem webConsole = new JMenuItem("Web 控制台");
...
//		configMenu.add(dTypeMenuItem);
		...
		// 在浏览器中打开 Web 控制台（现代化导航页）
		webConsole.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 确保本地 HTTP 服务器已启动（默认仅在登录/Cookie 刷新时按需启动）
				Global.ensureWebServerStarted();
				String url = String.format("http://localhost:%d/console/index.html?lang=%s",
						Global.serverPort, Global.frontendLangCode());
				try {
					if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
						...
						JOptionPane.showMessageDialog(null,
								...
								"请注意", JOptionPane.WARNING_MESSAGE);
					}
				} catch (Exception ex) {
					ex.printStackTrace();
				}
