...
//			HttpsURLConnection.setDefaultSSLSocketFactory(TrustAllCertSSLUtil.getFactory());
	...
	public void init() {
		new Thread(new Runnable() {
			@Override
			public void run() {
				if (Global.persistentSocketServer != null) {
					socketServer = Global.persistentSocketServer; // 复用已启动的 Web 控制台服务器
					return;
				}
			...
			public void windowClosing(WindowEvent e) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (socketServer != null && socketServer != Global.persistentSocketServer)
	...
	private void queryCaptcha() {
		...
		if (Global.userName == null || Global.password == null || Global.userName.isEmpty()
			...
			try {
				...
				String url = String.format("http://localhost:%d/static/index.html?token=%s&gt=%s&challenge=%s&lang=%s", Global.serverPort,
						token, gt, challenge, Global.frontendLangCode());
